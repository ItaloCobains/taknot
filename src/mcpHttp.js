...
async function startMcpHttp(opts = {}) {
  ...
  if (!vault) throw new Error('startMcpHttp: vault is required');
  const version = opts.version || '0.0.0';
  const preferred =
    Number(opts.port) ||
    Number(process.env.TAKNOT_MCP_PORT) ||
    DEFAULT_PORT;
  ...
  app.post('/mcp', async (req, res) => {
    ...
    try {
      ...
      console.error('[taknot-mcp] request failed', error);
      if (!res.headersSent) {
        res.status(500).json({
          jsonrpc: '2.0',
  ...
  state.running = true;
  state.port = port;
  state.url = `http://${HOST}:${port}/mcp`;
  console.log('[taknot-mcp] listening', state.url);
